...
def _masked_mean(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    if not bool((denom > 0).detach().cpu()):
        raise ValueError("Multi-species timestep loss has zero active entries.")
    return (x * mask).sum() / denom


def _masked_max_abs(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    mask = mask.to(dtype=torch.bool, device=x.device)
    while mask.ndim < x.ndim:
        mask = mask.unsqueeze(0)
    mask = mask.expand_as(x)
    selected = torch.abs(x)[mask]
    if selected.numel() == 0:
        raise ValueError("Multi-species timestep diagnostics have zero active entries.")
    return selected.max()


def compute_timestep_consistency_loss_multispecies(
    model,
    ...
    *,
    ...
    if dt is None:
        if not hasattr(params, "dt") or params.dt is None:
    ...
    n_pairs = t0.numel()
    active = active_grid_mask(params).to(dtype=torch.bool, device=device)
    active_float = active.to(dtype=dtype, device=device)
    ...
    if loss_form == "physical":
        loss_timestep = _masked_mean(residual_physical ** 2, active)
    elif loss_form == "log":
        loss_timestep = _masked_mean(residual_log ** 2, active)
    elif loss_form == "relative":
        loss_timestep = _masked_mean(residual_relative ** 2, active)
    else:
    ...
    diagnostics: dict[str, torch.Tensor | str | bool] = {
        ...
        "physical_abs_mean": _masked_mean(torch.abs(residual_physical), active),
        "physical_abs_max": _masked_max_abs(residual_physical, active),
        "log_abs_mean": _masked_mean(torch.abs(residual_log), active),
        "log_abs_max": _masked_max_abs(residual_log, active),
        "relative_abs_mean": _masked_mean(torch.abs(residual_relative), active),
        "relative_abs_max": _masked_max_abs(residual_relative, active),
        "active_fraction": active_fraction.detach(),
        "active_count": active_count.detach(),
        "n_species": torch.as_tensor(active.shape[0], dtype=dtype, device=device),
