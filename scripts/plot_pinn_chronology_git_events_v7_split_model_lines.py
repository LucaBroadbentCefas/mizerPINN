...
SCRIPT_VERSION = "2026-07-28-v7-split-single-and-multispecies-lines"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a chronological overview of PINN runs, separating single-species "
            "and multispecies experiments."
        )
    )
    parser.add_argument(
        "--run-root",
    ...
    parser.add_argument(
        "--output-dir",
        default="analysis/pinn_chronology_git_v7",
    )
    parser.add_argument(
...
def detect_model_family(
    ...
    """Classify a run without relying on its parent root alone.

    The command/entrypoint is the strongest signal. Path names and saved
    species metadata are fallbacks for older runs that did not preserve the
    full command.
    """
    command_text = ""
    ...
    path_text = str(run_dir).lower()

    multispecies_tokens = (
        "train_pde_multispecies",
        "pde_multispecies",
        "multi_species",
        "multispecies",
    )
    single_species_tokens = (
        "train_pde_only_single_species",
        "pde_only_single_species",
        "single_species",
        "single-species",
    )

    # Entrypoint wins over the storage path. This handles multispecies runs
    # that were accidentally saved beneath a historically single-species root.
    if any(token in command_text for token in multispecies_tokens):
        return "Multispecies"
    if any(token in command_text for token in single_species_tokens):
        return "Single species"

    n_species = effective_config.get("n_species")
    try:
        if n_species is not None:
            return "Multispecies" if int(n_species) > 1 else "Single species"
    except (TypeError, ValueError):
        pass

    species_names = effective_config.get("species_names")
    if isinstance(species_names, (list, tuple)):
        return "Multispecies" if len(species_names) > 1 else "Single species"

    if any(token in path_text for token in multispecies_tokens):
        return "Multispecies"
    if any(token in path_text for token in single_species_tokens):
        return "Single species"

    # Do not force an uncertain run into either scientific comparison line.
...
def build_daily_table(
    ...
    """Summarise each model family independently on each date."""
    daily_rows: list[dict[str, Any]] = []
    previous_modes_by_family: dict[str, dict[str, Any]] = {}

    grouped = runs.groupby(["model_family", "date"], sort=True)
    for (model_family, date), day in grouped:
        ...
        previous_modes = previous_modes_by_family.get(model_family)
        change_items = []
        ...
        previous_modes_by_family[model_family] = modes

    daily = pd.DataFrame(daily_rows)
    if not daily.empty:
        daily = daily.sort_values(["model_family", "date_dt"]).reset_index(drop=True)
        daily["cumulative_best_loss"] = daily.groupby(
            "model_family", sort=False
        )["best_standardised_loss"].cummin()
    return daily


def annotate_n_steps_changes(ax: plt.Axes, daily: pd.DataFrame) -> None:
    for model_family, family_daily in daily.groupby("model_family", sort=True):
        previous = None
        for _, row in family_daily.sort_values("date_dt").iterrows():
            ...
            if previous is not None and value_key(previous) != value_key(current):
                ...
                ax.axvline(when, linestyle="--", linewidth=0.8, alpha=0.28)
                ax.text(
                    when,
                    0.98,
                    f"{model_family}: steps {format_value(previous)}→{format_value(current)}",
                    rotation=90,
                    va="top",
                    ha="right",
                    fontsize=7.5,
...
def _build_chronology_cards(
    ...
    """Build point-specific model cards plus separate repository-event cards."""
    cards: list[dict[str, Any]] = []

    for _, day in daily.sort_values(["date_dt", "model_family"]).iterrows():
        best = float(day["best_standardised_loss"])
        best_text = f"{best:.2e}" if math.isfinite(best) else "not comparable"
        change_text = str(day["changes_from_previous_day"])
        if change_text == "none":
            change_text = "Initial recorded configuration for this model family"
        ...
        body_lines = [
            ...
            f"Changed: {change_text}.",
            f"Tested: {day['varied_within_day']}.",
        ]
        ...
        cards.append(
            {
                "date": str(day["date"]),
                "date_dt": pd.Timestamp(day["date_dt"]),
                "title": (
                    f"{pd.Timestamp(day['date_dt']).strftime('%d %b %Y')} — "
                    f"{day['model_family']}"
                ),
                "body": "\n".join(wrapped_lines),
                "line_count": max(1, len(wrapped_lines)),
                "has_event": False,
                "has_daily_point": math.isfinite(best),
                "daily_best": best,
                "model_family": str(day["model_family"]),
            }
        )
            ...
            cards.append(
                {
                    "date": str(date),
                    "date_dt": pd.Timestamp(date),
                    "title": f"{pd.Timestamp(date).strftime('%d %b %Y')} — Git event",
                    "body": "\n".join(wrapped_lines),
                    "line_count": max(1, len(wrapped_lines)),
                    "has_event": True,
                    "has_daily_point": False,
                    "daily_best": math.nan,
                    "model_family": "Repository event",
                }
            )

    return sorted(
        cards,
        key=lambda item: (item["date_dt"], item["model_family"]),
    )
...
def _place_collision_aware_cards(
    *,
    ...
    ordered_cards = sorted(
        cards,
        key=lambda item: (item["date_dt"], item.get("model_family", "")),
    )
    ...
    for index, (card, (box_width, box_height)) in enumerate(
        ...
        has_point_anchor = bool(card.get("has_daily_point", False))
        point_anchor_y = float(card.get("daily_best", math.nan))
...
def plot_chronology(
    ...
    fig.suptitle(
        "PINN development chronology: single-species vs multispecies",
        fontsize=17,
        fontweight="bold",
        y=0.995,
    )
    ...
    colour_cycle = plt.rcParams["axes.prop_cycle"].by_key().get(
        "color", ["C0", "C1", "C2"]
    )
    model_order = ["Single species", "Multispecies", "Unknown model"]
    model_colours = {
        model: colour_cycle[index % len(colour_cycle)]
        for index, model in enumerate(model_order)
    }

    for model_family in model_order:
        group = eligible.loc[eligible["model_family"] == model_family]
        if group.empty:
            continue
        ax_loss.scatter(
            group["run_time"],
            ...
            s=48,
            alpha=0.48,
            color=model_colours[model_family],
            label=f"{model_family}: individual runs",
            zorder=3,
        )

    valid_daily = daily.loc[np.isfinite(daily["best_standardised_loss"])].copy()
    for model_family in model_order:
        family_daily = valid_daily.loc[
            valid_daily["model_family"] == model_family
        ].sort_values("date_dt")
        if family_daily.empty:
            continue

        family_x = family_daily["date_dt"] + pd.to_timedelta(12, unit="h")
        colour = model_colours[model_family]
        marker = model_markers[model_family]

        ax_loss.plot(
            family_x,
            family_daily["best_standardised_loss"],
            marker=marker,
            linewidth=2.4,
            color=colour,
            label=f"{model_family}: best run each day",
            zorder=6,
        )
        ax_loss.plot(
            family_x,
            family_daily["cumulative_best_loss"],
            linestyle="--",
            linewidth=1.8,
            color=colour,
            alpha=0.82,
            label=f"{model_family}: best achieved so far",
            zorder=5,
        )
    ...
    fig.text(
        ...
        0.012,
        (
            "Single-species and multispecies runs are aggregated and connected separately. "
            "Each model card connects to its own exact daily-best point. Repository events "
            "connect to the plot boundary, and all cards remain in reserved white space."
        ),
        ha="center",
        fontsize=9,
    )
...
def plot_daily_argument_table(daily: pd.DataFrame, output_path: Path) -> None:
    ...
    rendered = ax.table(
        ...
        colLoc="left",
        loc="center",
        colWidths=[0.09, 0.065, 0.04, 0.055, 0.075, 0.07, 0.29, 0.31],
    )
    ...
    ax.set_title(
        "Daily experiment chronology by model family",
        fontsize=15,
        fontweight="bold",
        pad=14,
    )
...
def main() -> None:
    ...
    plot_chronology(
        ...
        daily,
        compare_step=args.compare_step,
        output_path=output_dir / "01_pinn_chronology_by_model.png",
        git_events=events,
    )
    ...
    print(f"Model-days represented: {len(daily)}")
    print("\nDaily summary:")
    print(
        daily[
            [
