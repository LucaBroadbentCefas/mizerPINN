...
def assemble(*, runs_root: str | Path, input_dir: str | Path, known_state_csv: str | Path,
             output_dir: str | Path, dtype: torch.dtype = torch.float64, device: str = "cpu") -> Path:
    ...
    for species_idx, species_name in enumerate(params.species):
        ...
        models.append(model); run_paths.append(str(run))
    ...
    prediction = evaluate_log_model_on_points(ensemble, scale_x(x, params), scale_t(t, params), params)
    known_n = known.at(t)
    ...
    pd.DataFrame({"time":tt,"species_idx":ss,"weight":ww,
        "N_pred":prediction["N"].detach().cpu().reshape(-1).numpy(),
        "N_known":known_n.detach().cpu().reshape(-1).numpy()}).to_csv(output / "ensemble_predictions.csv", index=False)
    return output


def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--runs-root",required=True); p.add_argument("--input-dir",required=True)
    p.add_argument("--known-state-csv",required=True); p.add_argument("--output-dir",required=True)
    a=p.parse_args(argv); print(assemble(**vars(a)))
